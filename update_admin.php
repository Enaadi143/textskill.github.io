...
{
$sn++;
header("location:check.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
}
...
header("location:check.php?q=result&eid=$eid");
}
else
{
header("location:check.php?q=result&eid=$eid");
...
$sun=$sun-$s;
$q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
header("location:check.php?q=quiz&step=2&eid=$eid&n=1&t=$t");
}
